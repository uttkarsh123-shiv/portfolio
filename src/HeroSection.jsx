...
const HeroSection = ({ id }) => {
  ...
  return (
    <div id={id} className="w-screen md:h-screen flex flex-col gap-10">
      <Navbar />
      <div className="h-full w-full pt-10 md:pt-20  flex flex-col  border-gray-300 justify-between gap-5  items-center">
        ...
        <div className="button-grp gap-2 md:gap-5 flex pt-2 md:pt-10">
          <button className="border pl-3 md:pl-7 pr-3 md:pr-7 p-1 md:p-3 rounded-full bg-black text-white cursor-pointer hover:bg-[#292929] text-[9px] md:text-[20px]"
 onClick={() => window.location.href = "mailto:uttkarshsingh450@gmail.com?subject=Collaboration Request"}>
            Let's Collaborate
          </button>
          <button   onClick={() => navigate("/work")} className="border border-gray-400  pl-3 md:pl-6 pr-3 md:pr-6 p-1 md:p-3 rounded-full cursor-pointer text-[9px] md:text-[20px]">
            See Work
          </button>
        </div>
      </div>
